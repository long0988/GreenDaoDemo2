...
public class ArcProgress extends ProgressBar {
    ...
    private LinearGradient linearGradient, linearGradient2;
    ...
    public ArcProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        linearGradient = new LinearGradient(
                0f, 0f, 300, 0f, new int[]{getResources().getColor(R.color.end), getResources().getColor(R.color.start)}, null, Shader.TileMode.CLAMP
        );
        linearGradient2 = new LinearGradient(
                0f, 0f, 300, 0f, new int[]{mUnmProgressColor, mUnmProgressColor}, null, Shader.TileMode.CLAMP
        );
    }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        } else if (mStylePogress == STYLE_GRADIENT) {
            float targetmDegree = (360 - mDegree) * roate;
            //绘制完成部分
            mArcPaint.setShader(linearGradient);
            canvas.drawArc(mArcRectf, 90 + angle, targetmDegree, false, mArcPaint);
            //绘制未完成部分
            mArcPaint.setShader(linearGradient2);
            canvas.drawArc(mArcRectf, 90 + angle + targetmDegree, 360 - mDegree - targetmDegree, false, mArcPaint);
        } else {
            if (mBgShow)
